...
#cap = cv2.VideoCapture('Astra Pro HD Camera')
...
## save screen position file in local
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
screen_position_file = os.path.join(BASE_DIR, "screen_position.txt")

## normalize gaze with timestamp
def write_screen_position(x, y):
    u = x / max(1, MONITOR_WIDTH)
    v = y / max(1, MONITOR_HEIGHT)
    t = time.time()
    with open(screen_position_file, 'w') as f:
        f.write(f"{t:.6f},{u:.6f},{v:.6f}\n")
